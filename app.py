...
def render_risk_meter(confidence, is_threat):
    color = "#ff0000" if is_threat and confidence > 80 else "#ffbf00" if is_threat else "#00ff41"
    st.markdown(f"""
...
with tab1:
    ...
    presets = {
        "Manual": None,
        "Normal Web": {"dur": 60000, "pkts": 12, "len_max": 1200, "len_mean": 450, "bytes": 500},
        "DDoS Flood": {"dur": 150, "pkts": 5000, "len_max": 0, "len_mean": 0, "bytes": 100000},
        "Port Scan": {"dur": 50, "pkts": 2, "len_max": 0, "len_mean": 0, "bytes": 40}
    }
    
    selected_preset = st.selectbox("📝 Load Traffic Profile", list(presets.keys()))
    defaults = presets[selected_preset] if selected_preset != "Manual" else {"dur": 500, "pkts": 10, "len_max": 60, "len_mean": 50.0, "bytes": 100}
...
with tab3:
    ...
    if len(st.session_state.history) > 0:
        ...
        with c1:
            st.caption("Threat vs Safe Traffic")
        ...
        with c2:
            st.caption("Engine Usage")
            st.bar_chart(df_hist['Model'].value_counts())
            
        st.markdown("### 📜 Activity Log")
        st.dataframe(df_hist, use_container_width=True)
    else:
        st.info("No data yet. Run a scan in the Live Simulator tab.")
...
with tab4:
    ...
    model = models_dict["RandomForest"]
    
    if hasattr(model, 'feature_importances_'):
        importances = model.feature_importances_
        # Safe Mapping
        names = FEATURE_COLS if len(importances) == len(FEATURE_COLS) else [f"Feat {i}" for i in range(len(importances))]
        df_imp = pd.DataFrame({'Feature': names, 'Importance': importances}).sort_values('Importance', ascending=False).head(10)
